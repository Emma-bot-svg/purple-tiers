...
export default function Home() {
  ...
  const [serverIp, setServerIp] = useState('PLAY.YUGEMPIRE.COM') 
  ...
  const getRankTag = (points: number, rankIndex: number) => {
    if (rankIndex === 0) return { name: 'Combat Grandmaster', img: 'https://mctiers.com/titles/combat_grandmaster.webp' };
  ...
  const getLB = () => {
    ...
    players.forEach(p => {
      ...
      if (!g[n]) g[n] = { username: n, points: 0, region: p.region || 'AS', tiers: {} }
      g[n].points += (p.points || 0)
        ...
        {activeGamemode === 'Overall' ? (
          <div className="flex flex-col gap-2.5">
            {getLB().map((p: any, i) => {
              ...
              const testedModes = modes.slice(1).sort((a, b) => (p.tiers[b.n] ? 1 : 0) - (p.tiers[a.n] ? 1 : 0));
              return (
